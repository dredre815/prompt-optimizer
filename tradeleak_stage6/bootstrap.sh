...
set -Eeuo pipefail
umask 077
ISSUE_NUMBER="${TRADELEAK_STAGE6_ISSUE_NUMBER:-51}"; ATA_COMMIT="${ATA_COMMIT:-6b49ee837ebc1fa0d5bf99d655dc3adc352a77d3}"; NONCE="${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}-$(date +%s)"
ROOT="$(mktemp -d /tmp/tl6-XXXXXX)"; PRIV="$ROOT/private.pem"; PUB="$ROOT/public.pem"; CHEX="$ROOT/c.hex"; CBIN="$ROOT/c.bin"; KEYF="$ROOT/key"; RESULTS="$GITHUB_WORKSPACE/tradeleak_stage6_output"; mkdir -p "$RESULTS"; rm -rf "$RESULTS"/*
...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096 -out "$PRIV" 2>/dev/null;openssl pkey -in "$PRIV" -pubout -out "$PUB";{ echo TRADELEAK_STAGE6_PUBLIC_KEY_V2;echo "nonce=$NONCE";echo "run_id=$GITHUB_RUN_ID";echo requested_model=deepseek-v4-flash;echo "ata_commit=$ATA_COMMIT";echo TRADELEAK_STAGE6_PUBLIC_KEY_BEGIN;cat "$PUB";echo TRADELEAK_STAGE6_PUBLIC_KEY_END;} > "$ROOT/pub";comment "$ROOT/pub"
...
import json,re,sys
src,nonce,out,cid=sys.argv[1:];xs=json.load(open(src));head=re.compile(r'TRADELEAK_STAGE6_CIPHERTEXT_CHUNKS_V2\s*\nnonce='+re.escape(nonce)+r'\s*\n')
...
git clone --quiet https://github.com/MTMQuantAI/Agent-Trading-Arena.git "$ROOT/ata";git -C "$ROOT/ata" checkout --quiet "$ATA_COMMIT";export ATA_ROOT="$ROOT/ata";export TRADELEAK_STAGE6_OUT="$RESULTS";python "$GITHUB_WORKSPACE/tradeleak_stage6/runner.py"|tee "$RESULTS/execution.log";echo "$ATA_COMMIT">"$RESULTS/ata_commit.txt"
...
import json,sys,os
s=json.load(open(sys.argv[1]));keys=['recon_accuracy','selection_precision_for_s1','selection_recall_for_s1','blind_random_target_success_rate','tradeleak_selected_success_rate','oracle_s1_success_rate','blind_cost_per_success','tradeleak_cost_per_success','oracle_cost_per_success','cost_reduction_factor_vs_blind','false_positive_spend','valid_response_rate'];open(sys.argv[2],'w').write('\n'.join(['TRADELEAK_STAGE6_RESULT_V1',f"run_id={os.environ.get('GITHUB_RUN_ID')}",f"model={s['model']}"]+[f"{k}={s[k]}" for k in keys])+'\n')
PY
