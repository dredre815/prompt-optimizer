...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096 -out "$PRIV" 2>/dev/null;openssl pkey -in "$PRIV" -pubout -out "$PUB";{ echo TRADELEAK_STAGE5C_PUBLIC_KEY_V2;echo "nonce=$NONCE";echo "run_id=$GITHUB_RUN_ID";echo requested_model=deepseek-v4-flash;echo "ata_commit=$ATA_COMMIT";echo TRADELEAK_STAGE5C_PUBLIC_KEY_BEGIN;cat "$PUB";echo TRADELEAK_STAGE5C_PUBLIC_KEY_END;} > "$ROOT/pub";comment "$ROOT/pub"
...
import json,re,sys
src,nonce,out,cid=sys.argv[1:];xs=json.load(open(src))
head=re.compile(r'TRADELEAK_STAGE5C_CIPHERTEXT_CHUNKS_V2\s*\nnonce='+re.escape(nonce)+r'\s*\n')
...
raise SystemExit(1)
PY
then found=1;break;fi;sleep 5;done;[ "$found" -eq 1 ]||exit 2;chars="$(tr -d '\n\r ' < "$CHEX" | wc -c)";[ "$chars" -eq 1024 ]||{ echo "ciphertext hex length=$chars expected=1024"; exit 4; };xxd -r -p "$CHEX">"$CBIN";bytes="$(wc -c < "$CBIN")";[ "$bytes" -eq 512 ]||{ echo "ciphertext bytes=$bytes expected=512"; exit 5; };openssl pkeyutl -decrypt -inkey "$PRIV" -in "$CBIN" -out "$KEYF" -pkeyopt rsa_padding_mode:oaep -pkeyopt rsa_oaep_md:sha256 -pkeyopt rsa_mgf1_md:sha256
