...
ROOT="$(mktemp -d /tmp/tradeleak-native2048-XXXXXX)"
...
trap cleanup EXIT
comment_file() { GH_TOKEN="$GITHUB_TOKEN" gh issue comment "$ISSUE_NUMBER" --repo "$GITHUB_REPOSITORY" --body-file "$1" >/dev/null; }
...
openssl pkey -in "$PRIVATE_KEY" -pubout -out "$PUBLIC_KEY"
{
  ...
  echo "purpose=native-ata-principle-full-cognition-2048-confirmation"
...
if [ -s "$CID_FILE" ]; then GH_TOKEN="$GITHUB_TOKEN" gh api -X DELETE "repos/${GITHUB_REPOSITORY}/issues/comments/$(cat "$CID_FILE")" >/dev/null 2>&1 || true; fi
...
for p in root.rglob('*'):
    ...
    raw=p.read_bytes(); new=raw.replace(secret,b'[REDACTED_DEEPSEEK_KEY]') if secret else raw; new=pat.sub(b'[REDACTED_API_KEY_PATTERN]',new)
...
lines=['TRADELEAK_NATIVE2048_RESULT_V1',f'run_id={os.environ.get("GITHUB_RUN_ID")}',f'exit_code={rc}',f'ata_commit={commit}']
try:
    s=json.load(open(summary_path,encoding='utf-8'))
    for k in ['verdict','model','max_tokens','matched_pairs','selected_probes','mean_passive_accuracy','mean_random_accuracy','mean_active_accuracy','active_gain_over_passive','active_gain_over_random','valid_response_rate','median_active_price_impact_pct','api_stats']:
        v=s.get(k); lines.append(f'{k}={json.dumps(v,ensure_ascii=False) if isinstance(v,(dict,list)) else v}')
    lines.append('pair_metrics='+json.dumps(s.get('pair_metrics'),ensure_ascii=False,separators=(',',':')))
except Exception as e: lines.append('summary_error='+repr(e))
