...
set -Eeuo pipefail
umask 077
ISSUE_NUMBER="${TRADELEAK_STAGE4_ISSUE_NUMBER:-41}"; ATA_COMMIT="${ATA_COMMIT:-6b49ee837ebc1fa0d5bf99d655dc3adc352a77d3}"; NONCE="${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}-$(date +%s)"
ROOT="$(mktemp -d /tmp/tl4-XXXXXX)"; PRIV="$ROOT/private.pem"; PUB="$ROOT/public.pem"; C64="$ROOT/c.b64"; CBIN="$ROOT/c.bin"; KEYF="$ROOT/key"; RESULTS="$GITHUB_WORKSPACE/tradeleak_stage4_output"; mkdir -p "$RESULTS"; rm -rf "$RESULTS"/*
...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096 -out "$PRIV" 2>/dev/null;openssl pkey -in "$PRIV" -pubout -out "$PUB";{ echo TRADELEAK_STAGE4_PUBLIC_KEY_V1;echo "nonce=$NONCE";echo "run_id=$GITHUB_RUN_ID";echo requested_model=deepseek-v4-flash;echo "ata_commit=$ATA_COMMIT";echo TRADELEAK_STAGE4_PUBLIC_KEY_BEGIN;cat "$PUB";echo TRADELEAK_STAGE4_PUBLIC_KEY_END;} > "$ROOT/pub";comment "$ROOT/pub"
...
import json,re,sys
src,nonce,out,cid=sys.argv[1:];xs=json.load(open(src));p=re.compile(r'TRADELEAK_STAGE4_CIPHERTEXT_V1\s*\nnonce='+re.escape(nonce)+r'\s*\nTRADELEAK_STAGE4_CIPHERTEXT_BEGIN\s*\n(.*?)\nTRADELEAK_STAGE4_CIPHERTEXT_END',re.S)
...
git clone --quiet https://github.com/MTMQuantAI/Agent-Trading-Arena.git "$ROOT/ata";git -C "$ROOT/ata" checkout --quiet "$ATA_COMMIT";export ATA_ROOT="$ROOT/ata";export TRADELEAK_STAGE4_OUT="$RESULTS";python "$GITHUB_WORKSPACE/tradeleak_stage4/runner.py"|tee "$RESULTS/execution.log";echo "$ATA_COMMIT">"$RESULTS/ata_commit.txt"
...
import json,sys,os
s=json.load(open(sys.argv[1]));open(sys.argv[2],'w').write('\n'.join(['TRADELEAK_STAGE4_RESULT_V1',f"run_id={os.environ.get('GITHUB_RUN_ID')}",f"verdict={s['verdict']}",f"model={s['model']}",f"valid_response_rate={s['valid_response_rate']}",f"metrics={json.dumps(s['metrics'],separators=(',',':'))}"])+'\n')
PY
