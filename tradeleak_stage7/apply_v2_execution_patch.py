...
replacements = [
    ...
    (
        ...
        "'scientific_design_changed':False,'execution_addendum':{'native_repeat_cap':NATIVE_REPEAT_CAP,'provider_attempts':PROVIDER_ATTEMPTS,'request_timeout_sec':REQUEST_TIMEOUT_SEC,'max_completion_tokens':MAX_COMPLETION_TOKENS,'semantic_parser_adapter':False},'api_stats':API_STATS,'gate':'ON recovery AUC>=0.75 AND OFF<=0.65 AND delta>=0.15 AND valid>=0.90','verdict':'GO' if go else 'NO-GO'}",
    ),
]
...
for old, new in replacements:
    count = s.count(old)
    if count != 1:
        raise RuntimeError(f"Stage-7 V2 patch expected exactly one match, got {count}: {old[:120]!r}")
    s = s.replace(old, new, 1)

p.write_text(s, encoding="utf-8")
print("Applied frozen Stage-7 V2 execution patch: retry cap + max_tokens=4096 + empty-content telemetry")
